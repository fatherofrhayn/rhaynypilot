...
class CarController():
  def __init__(self, dbc_name, CP, VM):
    ...
    self.vision_turn_speed_control = self.params.get_bool("TurnVisionControl")
    self.map_turn_speed_control = self.params.get_bool("TurnSpeedControl")
    self.last_speed_limit_sign_tap = self.params.get_bool("LastSpeedLimitSignTap")
    self.speed_limit = 0.
    self.stock_resume_alt = self.params.get_bool("StockResumeAlt")
    self.is_metric = self.params.get_bool("IsMetric")
    self.speed_limit_perc_offset = self.params.get_bool("SpeedLimitPercOffset")
    self.speed_limit_value_offset = int(self.params.get("SpeedLimitValueOffset"))
    self.signal_last = 0.
  ...
  def update(self, enabled, CS, frame, actuators, pcm_cancel_cmd, visual_alert, hud_speed,
    ...
    if frame % 2 == 0 and CS.CP.openpilotLongitudinalControl:
      ...
      can_sends.extend(create_acc_commands(self.packer, enabled and CS.out.cruiseState.enabled, accel, jerk_upper, jerk_lower, int(frame / 2), lead_visible, set_speed_in_units, stopping, tau_gap_set, CS.accMainOn))
      self.accel = accel
    ...
    if frame % 5 == 0 and self.car_fingerprint in [CAR.SONATA, CAR.PALISADE, CAR.IONIQ, CAR.KIA_NIRO_EV, CAR.KIA_NIRO_HEV_2021,
      ...
      can_sends.append(create_lfahda_mfc(self.packer, enabled, lkas_active, disengage_from_brakes, below_lane_change_speed, disengage_blinking_icon, slc_active, self.speed_limit, speed_limit_changed, switching_to_hda, zero_vego))
    ...
    if self.refresh_timer > 100:
      self.refresh_timer = 0
      self.speed_limit_control_enabled = self.params.get_bool("SpeedLimitControl")
      self.vision_turn_speed_control = self.params.get_bool("TurnVisionControl")
      self.map_turn_speed_control = self.params.get_bool("TurnSpeedControl")
      self.last_speed_limit_sign_tap = self.params.get_bool("LastSpeedLimitSignTap")
      self.speed_limit = self.get_speed_limit_osm()
  ...
  def get_speed_limit_osm(self):
    speed_limit_osm = float(self.sm['longitudinalPlan'].speedLimit if self.sm['longitudinalPlan'].speedLimit is not None else 0.0) * (CV.MS_TO_MPH if not self.is_metric else CV.MS_TO_KPH)
  ...
  def get_button_control(self, CS, final_speed):
    ...
    self.init_speed = round(min(final_speed, v_cruise_kph_max) * CV.KPH_TO_MPH) if not self.is_metric else round(min(final_speed, v_cruise_kph_max))
