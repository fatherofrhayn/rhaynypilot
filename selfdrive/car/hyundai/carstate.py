...
class CarState(CarStateBase):
  def __init__(self, CP):
    ...
    self.gap_adjust_cruise_tr = int(self.params.get("GapAdjustCruiseTr"))

    self.mads_enabled = self.params.get_bool("EnableMADS")
    self.gap_adjust_cruise = self.params.get_bool("GapAdjustCruise")

    self.acc_main_on = None
    self.prev_acc_main_on = None
  ...
  def update(self, cp, cp_cam):
    ...
    self.prev_acc_main_on = self.acc_main_on
    ...
    if self.CP.openpilotLongitudinalControl:
      self.acc_main_on = cp.vl["CLU11"]["CF_Clu_CruiseSwMain"] == 0
    ...
    if self.CP.openpilotLongitudinalControl:
      if self.prev_acc_main_on is None:
        self.prev_acc_main_on = self.acc_main_on
      ...
      if self.gap_adjust_cruise:
        if self.prev_cruise_buttons != 3: # GAP
          if self.cruise_buttons == 3:
            ...
            self.params.put("GapAdjustCruiseTr", str(self.gap_adjust_cruise_tr))
      else:
        self.gap_adjust_cruise_tr = 4
    ...
    # cruise state
    if self.CP.openpilotLongitudinalControl:
      if self.prev_acc_main_on != 1: #1 == not ACC Main button
        if self.acc_main_on == 1:
          self.accMainOn = not self.accMainOn
      # These are not used for engage/disengage since openpilot keeps track of state using the buttons
      ret.cruiseState.available = cp.vl["TCS13"]["ACCEnable"] == 0 or self.accMainOn
